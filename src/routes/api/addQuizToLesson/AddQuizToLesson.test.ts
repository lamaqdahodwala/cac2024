...
describe('AddQuizToLesson', () => {
    ...
    it('Can add a quiz to a lesson', async () => {
        ...
        await expect(apiRoute.callRoute(event)).resolves.toEqual({ message: 'Quiz added successfully', quiz: fakeQuiz });
    });
    ...
    it('Throws an error if user is not an admin', async () => {
        ...
        await expect(apiRoute.callRoute(event)).rejects.toThrow('Unauthorized: Only admins can add quizzes');
    });
    ...
    it('Throws an error if a quiz already exists for the lesson', async () => {
        ...
        await expect(apiRoute.callRoute(event)).rejects.toThrow('A quiz already exists for this lesson');
    });
});
