...
export class AddQuestionToQuiz implements RouteImplementation {
    async call(props: { prisma: PrismaClient; quizId: number; question: string; userId: string }): Promise<QuizQuestion> {
        ...
        return await props.prisma.$transaction(async (tx) => {
            ...
            await Promise.all(placeholderAnswers.map((answer, index) =>
                tx.quizAnswer.create({
                    data: {
                        quizQuestion: { connect: { id: quizQuestion.id } },
                        answerText: answer,
                        isCorrect: index === 0,
                        letterChoice: String.fromCharCode(65 + index), // A, B, C, D
                    }
                })
            ));
            return quizQuestion;
